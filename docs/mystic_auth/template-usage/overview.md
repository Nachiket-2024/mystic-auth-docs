...
Every setting is documented inline in [`env/.env.example`](https://github.com/Nachiket-2024/mystic-auth/blob/main/env/.env.example): treat that file as the source of truth, not this doc. `frontend/.env.example` only matters if you run the frontend locally with `npm run dev` instead of Docker.
