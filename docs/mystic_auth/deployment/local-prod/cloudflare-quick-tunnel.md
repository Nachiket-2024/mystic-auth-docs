...
- Plain `up -d backend frontend` isn't enough on its own either: Compose only recreates a container whose _own_ config changed, and frontend's config didn't - only backend's env did - so frontend would silently keep running unchanged, DNS cache and all.
