...
4. The protected account route (`PUT /users/{email}`, which reuses the same `UserUpdate` schema) does **not** require this, since a caller changing someone else's password already authenticated via their own `users:update_any` permission; requiring the _target's_ current password there would be nonsensical and isn't what the check is protecting against.
...
4. **Scope of the idempotency**: this is specifically about an already-dead _presented token_, not about whether the revoke Valkey was asked to perform actually happened.
...
5. **Also covered**:
   - The permission-protected path, not just self-service: a caller changing a _different_ account's password (`PUT /users/{email}`) revokes that target's sessions, so the target's own browser is the one left holding a dead refresh-token cookie; logging out from the target's session must succeed the same way.
