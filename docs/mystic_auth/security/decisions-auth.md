...
**This was reviewed and kept as-is, deliberately.** The alternative: fail _open_ (treat a Redis outage as "unlimited," letting every request through unthrottled): would silently disable brute-force and credential-stuffing protection across the entire authentication surface at precisely the moment (infrastructure instability) an attacker is statistically most likely to be probing for exactly that kind of gap. A temporary full-surface `429` is recoverable and visible (users see errors, monitoring/alerting on 429 rates would catch it); a temporary silent removal of all rate limiting is neither. For a template whose purpose is an authentication _foundation_, fail-closed is the safer default to ship.
