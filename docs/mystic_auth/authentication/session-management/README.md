...
6. `POST /auth/logout` bumps that one session's `chain_ver` and marks the matching session row revoked (`session_service.revoke_session_on_logout`). It is functionally the same operation as a targeted Manage Sessions revoke, triggered by the device ending its own session.
7. `POST /auth/logout/all`, password reset confirm, password changes, account soft delete, and account purge all bump `account_ver` through `refresh_token_service.revoke_all_tokens_for_user`. One `INCR` ends every session on the account at once.
